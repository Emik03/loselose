...
public class WinWin : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        {
            Debug.LogFormat("[Win/Win #{0}] Strike! The module was not activated, yet you solved {1}.", _moduleId, Info.GetSolvableModuleNames().Last());
            _stage++;
            Module.HandleStrike();
        }
    }
            ...
            if (_stage < Info.GetSolvedModuleNames().Where(a => !_ignore.Contains(a)).Count() && !_isSolved)
            {
                ...
                if (allObjects[rng] != null)
                {
                    ...
                    for (int i = 0; i < _blacklist.Length; i++)
                    {
                        ...
                        {
                            //actual payload
                            Debug.LogFormat("[Win/Win #{0}] {1}", _moduleId, allObjects[rng].name);
                            Text.text = allObjects[rng].name.ToUpper();
                            Text.fontSize = 750 / allObjects[rng].name.Length;
                            Text.color = new Color32(0, 255, 255, 255);
                            _builder.Append(allObjects[rng].name + ", ");
                            Instantiate(allObjects[rng]);
                        }
                    }
    ...
    private void Solve()
    {
        Debug.LogFormat("[Win/Win #{0}] Defused! Module Solved!", _moduleId);
        Text.text = "";
        ...
        if (_builder.Length > 2)
        {
            ...
            Debug.LogFormat("[Win/Win #{0}] I have deleted: {1}", _moduleId, _builder);
        }

        //if nothing was destroyed, i.e. modules consisted of only ignore array
        else
            Debug.LogFormat("[Win/Win #{0}] I have deleted nothing.", _moduleId);
...
#pragma warning restore 414
    ...
    {
        yield return null;
        Debug.LogFormat("[Win/Win #{0}] Forced to be solved, initiating...", _moduleId);
        Solve();
    }
}